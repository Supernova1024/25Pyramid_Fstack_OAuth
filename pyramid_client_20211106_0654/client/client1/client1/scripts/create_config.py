...
def main():
    ...
    parser.add_argument(
        "--mysql_schema",
        default="client1",
        help="MySQL schema to use. Default to 'client1'",
    )
    parser.add_argument(
        "--mysql_user_name",
        required=True,
    ...
    client1_path = "."
    if not os.path.exists(os.path.join(client1_path, "templates")):
        print("You need to execute this in the directory client1")
        exit(1)
    ...
    template_environment = Environment(
        autoescape=False,
        loader=FileSystemLoader(os.path.join(client1_path, "templates")),
        trim_blocks=False,
    )
    ...
    if not os.path.exists(args.ini_path):
        ...
        print("client1 INI file created at {}".format(args.ini_path))
    else:
